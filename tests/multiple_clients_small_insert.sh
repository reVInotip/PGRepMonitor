...
psql -h localhost -p 5432 -U ubuntu -d postgres -f $PWD/tests/sql/create_test_db.sql

tps_values=()
latency_values=()

for i in {1..10}; do
    result=$(pgbench -h localhost -U ubuntu -d postgres -p 5432 -c 100 -j 2 \
        -t 1 -f $PWD/tests/sql/small_insert.sql 2>&1)
